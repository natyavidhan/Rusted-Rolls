...
class Scene:
    ...
    def draw(self):
        ...
        for entity in self.entities:
            list(entity.values())[0].draw(self.screen, self.offset, self)
        
        for i in self.game_map.rects:
...
class Sprite:
    def __init__(self, x, y, width, height, angle):
        ...
        self.col_Rects = {
            "top": pygame.Rect(self.x+1, self.y-1, self.width-2, 1),
            "bottom": pygame.Rect(self.x+1, self.y+self.height, self.width-2, 1),
            "left": pygame.Rect(self.x-1, self.y+1, 1, self.height-2),
            "right": pygame.Rect(self.x+self.width, self.y+1, 1, self.height-2)
        }
    ...
    def draw(self, screen, offset, scene):
        if self.current_animation:
            if self.current_frame >= len(self.animations[self.current_animation]):
                self.current_frame = 0
            self.load(self.animations[self.current_animation][self.current_frame])
            self.current_frame += 1
        x = self.x - offset[0] - self.origin[0]
        y = self.y - offset[1] - self.origin[1]
        self.rect = pygame.Rect(x, y, self.width, self.height)

        self.col_Rects = {
            "top": pygame.Rect(x+2, y-1, self.width-4, 1),
            "bottom": pygame.Rect(x+4, y+self.height, self.width-4, 1),
            "left": pygame.Rect(x-1, y+2, 1, self.height-4),
            "right": pygame.Rect(x+self.width, y+2, 1, self.height-4)
        }
        
        i=0
        for col in self.col_Rects:
            if self.col_Rects[col].collidelist(scene.game_map.rects) != -1:
